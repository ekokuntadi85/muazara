<div class="container mx-auto p-4 dark:bg-gray-800 dark:text-gray-200">
    ...
    <div class="flex flex-col-reverse md:flex-row md:justify-between md:items-center mb-4 space-y-4 md:space-y-0">
        ...
        <button type="button" wire:click="createSupplier()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline w-full md:w-auto dark:bg-blue-600 dark:hover:bg-blue-700 md:ml-4">Tambah Supplier</button>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                    ...
                    <tr class="dark:hover:bg-gray-700">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <button wire:click="edit({{ $supplier->id }})" class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-3 rounded-full mr-2 dark:bg-green-600 dark:hover:bg-green-700">Edit</button>
                            <button wire:click="delete({{ $supplier->id }})" onclick="confirm('Apakah Anda yakin ingin menghapus supplier ini?') || event.stopImmediatePropagation()" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded-full dark:bg-red-600 dark:hover:bg-red-700">Hapus</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <div class="bg-white dark:bg-gray-700 shadow-md rounded-lg p-4 border border-gray-200 dark:border-gray-600">
            <div class="flex justify-between items-start">
                ...
                <div class="flex space-x-2">
                    <button wire:click="edit({{ $supplier->id }})" class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-3 rounded-full text-xs dark:bg-green-600 dark:hover:bg-green-700">Edit</button>
                    <button wire:click="delete({{ $supplier->id }})" onclick="confirm('Apakah Anda yakin ingin menghapus supplier ini?') || event.stopImmediatePropagation()" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded-full text-xs dark:bg-red-600 dark:hover:bg-red-700">Hapus</button>
                </div>
            </div>
        </div>
        @empty
